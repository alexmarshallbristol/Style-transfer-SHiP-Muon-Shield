...
def remove_kinematic_information(inputs):
...
def _loss_generator(y_true, y_pred):
	...
	y_pred = tf.py_func(remove_kinematic_information,[y_pred],tf.float32)
...
discriminator.compile(loss=customLoss(d_input),optimizer=optimizerD) 
...
for e in range(epochs):
	...
	d_loss_gen = discriminator.train_on_batch(synthetic_output, gen_labels) # Train D
