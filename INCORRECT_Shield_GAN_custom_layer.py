...
def modgrad(op, grad):
    ...
    return grad * 1, grad * tf.negative(tf.floordiv(x, y)), grad
...
class MyLayer(Layer):
	...
	def call(self, inputs, **kwargs):
		...
		physics_array_processed = py_func(post_process_single,[physics_array],tf.float32,grad=modgrad)
		physics_array_processed.set_shape(physics_array.get_shape())

		initial_momentum = py_func(get_momentum_tensor,[physics_array_processed,0],tf.float32,grad=modgrad)
		final_momentum = py_func(get_momentum_tensor,[physics_array_processed,1],tf.float32,grad=modgrad)
		initial_momentum.set_shape(current_values.get_shape())
		final_momentum.set_shape(current_values.get_shape())

		new_values = py_func(get_new_values,[initial_momentum, final_momentum, current_values],tf.float32,grad=modgrad)
