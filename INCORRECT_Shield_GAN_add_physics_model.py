...
new_d_output = check_physics([output_of_D, physics_input])

physics_model = Model(inputs=[physics_input,output_of_D], outputs=[new_d_output])
...
muon_data = concatenate([inital_state_to_append, H],axis=1)

new_gan_output = physics_model(inputs=[muon_data,gan_output])

GAN_stacked = Model(inputs=[initial_state,noise_dims,inital_state_to_append], outputs=[new_gan_output])
GAN_stacked.compile(loss=_loss_generator, optimizer=optimizerD)
GAN_stacked.summary()


https://stackoverflow.com/questions/43277461/skipping-layer-in-backpropagation-in-keras
