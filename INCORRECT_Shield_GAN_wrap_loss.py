...
running_on = running_on_choices[3] 
...
def post_process_single(new_values):

	return new_values


def customLoss(d_input, y_pred_placeholder):

	# physics

	# new_values = y_pred_placeholder

	# physics_array = tf.py_func(post_process_single,[d_input, new_values],tf.float32)
	# physics_array.set_shape(new_values.get_shape())


	def _binary_crossentropy(y_true, y_pred):

		# physics_array = tf.py_func(post_process_single,[y_pred],tf.float32)
		# physics_array.set_shape(y_pred.get_shape())


		# physics_array = y_pred * 10

		# keras.backend.update(x, new_x)

		# result = K.mean(K.binary_crossentropy(y_true, y_pred), axis=-1)
		result = K.mean(K.binary_crossentropy(y_true, physics_array), axis=-1)

		return result

	loss_information = _binary_crossentropy
...
discriminator.compile(loss=customLoss(d_input, d_output),optimizer=optimizerD)
...
for e in range(epochs):
	...
	legit_labels = np.ones((int(batch_G), 1)) # Create label arrays
	gen_labels = np.zeros((int(batch_G), 1))
